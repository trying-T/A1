...
class RagService:
    ...
    def answer_repair(
        ...
        try:
            content = self.llm_service.generate_chat_completion(messages)
            parsed = self._parse_llm_json(content)
        except (LLMError, ValueError) as exc:
            parsed = self._fallback_answer(str(exc))
    ...
    def _fallback_answer(self, reason: str) -> dict[str, Any]:
        return {
            "fault_understanding": f"知识库中未找到充分依据，或模型调用失败：{reason}",
